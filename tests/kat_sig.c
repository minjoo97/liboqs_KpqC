...
OQS_STATUS combine_message_signature(uint8_t **signed_msg, size_t *signed_msg_len, const uint8_t *msg, size_t msg_len, const uint8_t *signature, size_t signature_len, const OQS_SIG *sig) {
	fprintf(stderr, "[DBG] combine_message_signature: method_name = \"%s\"\n", sig->method_name);
	...
	else if (0 == strcmp(sig->method_name, "HAETAE-2") || 0 == strcmp(sig->method_name, "HAETAE-3") || 0 == strcmp(sig->method_name, "HAETAE-5")) {
		// signed_msg = signature || msg
		*signed_msg_len = signature_len + msg_len;
		*signed_msg = OQS_MEM_malloc(*signed_msg_len);
		if (*signed_msg == NULL) {
		...
		memcpy(*signed_msg + signature_len, msg, msg_len);
		return OQS_SUCCESS;
	} else if (0 == strcmp(sig->method_name, "AIMER-128s") || 0 == strcmp(sig->method_name, "AIMER-128f") || 0 == strcmp(sig->method_name, "AIMER-192s") || 0 == strcmp(sig->method_name, "AIMER-192f") || 0 == strcmp(sig->method_name, "AIMER-256s") || 0 == strcmp(sig->method_name, "AIMER-256f")) {
		// signed_msg = signature || msg for AIMer
	*signed_msg_len = msg_len + signature_len;
    *signed_msg = OQS_MEM_malloc(*signed_msg_len);
    if (*signed_msg == NULL) {
        return OQS_ERROR;
