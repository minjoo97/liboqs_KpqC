...
#include "KpqClean_ver2_SMAUG-T3_clean/api.h"

#if defined(OQS_ENABLE_KEM_smaug_t3)
...
OQS_KEM *OQS_KEM_smaug_t3_new(void) {
    ...
    kem->method_name = OQS_KEM_alg_smaug_t3;
    kem->alg_version = "https://github.com/kpqc-cryptocraft/KpqClean_ver2/commit/80c525aa74d3b8983b16a124aa35dc61e50f43b7";

    kem->claimed_nist_level = 3;


    kem->length_public_key = OQS_KEM_smaug_t3_length_public_key;
    kem->length_secret_key = OQS_KEM_smaug_t3_length_secret_key;
    kem->length_ciphertext = OQS_KEM_smaug_t3_length_ciphertext;
    ...
    kem->decaps = OQS_KEM_smaug_t3_decaps;
...
OQS_API OQS_STATUS OQS_KEM_smaug_t3_keypair_derand(uint8_t *public_key, uint8_t *secret_key, const uint8_t *seed) {
	(void)public_key;
	(void)secret_key;
	(void)seed;
	return OQS_ERROR;
