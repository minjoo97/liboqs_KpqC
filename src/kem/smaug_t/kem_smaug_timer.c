...
#include "KpqClean_ver2_SMAUG-Timer_clean/api.h"


#if defined(OQS_ENABLE_KEM_smaug_timer)
...
OQS_KEM *OQS_KEM_smaug_timer_new(void) {
    ...
    kem->method_name = OQS_KEM_alg_smaug_timer;
    ...
    kem->length_public_key = OQS_KEM_smaug_timer_length_public_key;
    kem->length_secret_key = OQS_KEM_smaug_timer_length_secret_key;
    kem->length_ciphertext = OQS_KEM_smaug_timer_length_ciphertext;
    kem->length_shared_secret = OQS_KEM_smaug_timer_length_shared_secret;
    kem->length_keypair_seed = OQS_KEM_smaug_timer_length_keypair_seed;

    kem->keypair = OQS_KEM_smaug_timer_keypair;
    kem->keypair_derand = OQS_KEM_smaug_timer_keypair_derand;
    kem->encaps = OQS_KEM_smaug_timer_encaps;
    kem->decaps = OQS_KEM_smaug_timer_decaps;

    return kem;
}


OQS_API OQS_STATUS OQS_KEM_smaug_timer_keypair_derand(uint8_t *public_key, uint8_t *secret_key, const uint8_t *seed) {
	(void)public_key;
	(void)secret_key;
	(void)seed;
	return OQS_ERROR;
