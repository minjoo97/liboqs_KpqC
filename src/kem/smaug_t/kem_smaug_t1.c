...
#include "KpqClean_ver2_SMAUG-T1_clean/api.h"

#if defined(OQS_ENABLE_KEM_smaug_t1)
...
OQS_KEM *OQS_KEM_smaug_t1_new(void) {
    ...
    kem->method_name = OQS_KEM_alg_smaug_t1;
    ...
    kem->length_public_key = OQS_KEM_smaug_t1_length_public_key;
    kem->length_secret_key = OQS_KEM_smaug_t1_length_secret_key;
    kem->length_ciphertext = OQS_KEM_smaug_t1_length_ciphertext;
    kem->length_shared_secret = OQS_KEM_smaug_t1_length_shared_secret;
    kem->length_keypair_seed = OQS_KEM_smaug_t1_length_keypair_seed;


    printf("[SMAUG-T1] pk: %zu, sk: %zu, ct: %zu, ss: %zu\n",
        kem->length_public_key, kem->length_secret_key,
        kem->length_ciphertext, kem->length_shared_secret); 

    kem->keypair = OQS_KEM_smaug_t1_keypair;
    kem->keypair_derand = OQS_KEM_smaug_t1_keypair_derand;
    kem->encaps = OQS_KEM_smaug_t1_encaps;
    kem->decaps = OQS_KEM_smaug_t1_decaps;
...
OQS_API OQS_STATUS OQS_KEM_smaug_t1_keypair_derand(uint8_t *public_key, uint8_t *secret_key, const uint8_t *seed) {
	(void)public_key;
	(void)secret_key;
	(void)seed;
	return OQS_ERROR;
