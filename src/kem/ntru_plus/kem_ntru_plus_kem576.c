...
#include "KpqClean_ver2_NTRU_PLUS_KEM576_clean/api.h"
...
OQS_KEM *OQS_KEM_ntru_plus_kem576_new(void) {
    ...
    kem->method_name = OQS_KEM_alg_ntru_plus_kem576;
    ...
    kem->keypair = OQS_KEM_ntru_plus_kem576_keypair;
    kem->keypair_derand = OQS_KEM_ntru_plus_kem576_keypair_derand;
    kem->encaps = OQS_KEM_ntru_plus_kem576_encaps;
    kem->decaps = OQS_KEM_ntru_plus_kem576_decaps;
...
OQS_API OQS_STATUS OQS_KEM_ntru_plus_kem576_keypair_derand(uint8_t *public_key, uint8_t *secret_key, const uint8_t *seed) {
	(void)public_key;
	(void)secret_key;
	(void)seed;
	return OQS_ERROR;
