...
int crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m,
    ...
    xof256_absorb_twice(&state, sk, HAETAE_CRYPTO_PUBLICKEYBYTES, m, mlen);
...
int crypto_sign(uint8_t *sm, size_t *smlen, const uint8_t *m, size_t mlen,
    ...
    crypto_sign_signature(sm, smlen, sm + HAETAE_CRYPTO_BYTES, mlen, sk);
    *smlen += mlen;
    return 0;
}
...
#error "Not yet implemented."
    ...
    xof256_absorb_twice(&state, pk, HAETAE_CRYPTO_PUBLICKEYBYTES, m, mlen);
    xof256_squeeze(mu, SEEDBYTES, &state);
...
int crypto_sign_open(uint8_t *m, size_t *mlen, const uint8_t *sm, size_t smlen,
    ...
    *mlen = smlen - HAETAE_CRYPTO_BYTES;
    if (crypto_sign_verify(sm, HAETAE_CRYPTO_BYTES, sm + HAETAE_CRYPTO_BYTES, *mlen, pk))
        goto badsig;
    else {
